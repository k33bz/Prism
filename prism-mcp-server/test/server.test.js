...
function fixtureServer() {
  const server = new PrismMCPServer('(fixture)', { watch: false, logLevel: 'silent' });
...
test('tools/list returns 15 tools with schemas', async () => {
  const s = fixtureServer();
...
test('unknown tool yields -32602 with available tools', async () => {
  ...
  assert.ok(res.error.data.availableTools.length === 15);
});
